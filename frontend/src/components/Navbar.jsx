                ...
                <div className="hidden md:flex items-center gap-8">
                    ...
                    {user ? (
                        ...
                        <div className="flex items-center gap-4">
                            <Link to="/login" className="text-slate-600 font-bold hover:text-primary-600 transition-colors px-4 py-2">
                                {t('login')}
                            </Link>
                            <Link to="/signup" className="bg-primary-600 text-white px-6 py-2.5 rounded-2xl font-bold premium-shadow hover:bg-primary-700 transition-all active:scale-95 text-sm">
                                Get Started
                            </Link>
                        </div>
                    )}
                ...
                {isOpen && (
                    ...
                    >
                        <div className="p-6 flex flex-col gap-6">
                            <Link to="/discover" className="text-lg font-bold text-slate-900" onClick={() => setIsOpen(false)}>Explore</Link>
                            <button onClick={toggleLanguage} className="flex items-center gap-2 text-lg font-bold text-slate-600">
                            ...
                            {user ? (
                                <>
                                    <div className="pt-4 border-t border-slate-100">
                                        <div className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-2">My Account</div>
                                        <div className="text-xl font-bold text-slate-900 mb-4">{user.name}</div>
                                ...
                                <div className="flex flex-col gap-3 pt-4 border-t border-slate-100">
                                    <Link to="/signup" className="w-full py-4 bg-primary-600 text-white text-center rounded-2xl font-bold" onClick={() => setIsOpen(false)}>Join Heritage Farm</Link>
                                    <Link to="/login" className="w-full py-4 border border-slate-200 text-center rounded-2xl font-bold" onClick={() => setIsOpen(false)}>Sign In</Link>
                                </div>
                            )}
                        </div>
                    </motion.div>
