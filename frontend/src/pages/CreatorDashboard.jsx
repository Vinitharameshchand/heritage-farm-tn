...
const CreatorDashboard = () => {
    ...
    const [isKYCComplete, setIsKYCComplete] = useState(false); // Mock state, ideally should come from user object
    ...
    const fetchCreatorListings = async () => {
        try {
            setLoading(true);
            const response = await api.get('/listings'); // Filter by creator on backend or here
            const creatorListings = response.data.data.filter(l => l.creator === user?._id || l.creator?._id === user?._id);
    ...
    const stats = [
        { icon: Users, label: "Active Bookings", value: "12", trend: "+15%", color: "emerald" },
        { icon: TrendingUp, label: "Total Revenue", value: "₹45,000", trend: "+22%", color: "blue" },
        { icon: Eye, label: "Total Views", value: "1.2k", trend: "+8%", color: "purple" },
        { icon: Star, label: "Avg Rating", value: "4.9", trend: "+0.2", color: "amber" }
    ];
            ...
            {!isKYCComplete ? (
                ...
                <>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                        <div>
                            <h1 className="text-5xl font-outfit font-black mb-2">Creator <span className="gradient-text">Console</span></h1>
                            <p className="text-slate-500">Welcome back, {user?.name}. Here's your performance snapshot.</p>
                        </div>
                        <Link to="/creator/create" className="btn-primary px-8 py-4 flex items-center gap-3 shadow-xl shadow-emerald-500/20">
                        ...
                        <div className="overflow-x-auto">
                            {loading ? (
                                <div className="p-20 text-center">
                                    ...
                                    <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Syncing with Registry...</p>
                                </div>
                            ) : listings.length > 0 ? (
                                <table className="w-full text-left">
                                    <thead className="bg-white/5">
                                        <tr>
                                            <th className="px-8 py-5 text-xs font-black uppercase tracking-widest text-slate-500">Experience</th>
                                            <th className="px-8 py-5 text-xs font-black uppercase tracking-widest text-slate-500">Location</th>
                                            <th className="px-8 py-5 text-xs font-black uppercase tracking-widest text-slate-500">Price</th>
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {listings.filter(l => l.title.toLowerCase().includes(searchTerm.toLowerCase())).map((listing) => (
                                            <tr key={listing._id} className="hover:bg-white/5 transition-colors group">
                                                ...
                                                <td className="px-8 py-6 font-bold text-emerald-500">₹{listing.price}</td>
                                                <td className="px-8 py-6">
                                                ...
                                                <td className="px-8 py-6 text-right">
                                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <Link to={`/listings/${listing._id}`} className="p-3 bg-white/5 hover:bg-emerald-500/20 hover:text-emerald-500 rounded-xl transition-all" title="View">
                                                            <Eye className="w-4 h-4" />
                                                        </Link>
                                                        <Link to={`/creator/edit/${listing._id}`} className="p-3 bg-white/5 hover:bg-emerald-500/20 hover:text-emerald-500 rounded-xl transition-all" title="Edit">
                                ...
                                <div className="p-20 text-center">
                                    ...
                                    <p className="text-slate-500 max-w-xs mx-auto mb-8">Start sharing your culture and farm stories with the world.</p>
